...
void init_motors()
{
  softPwmCreate(RW, INITIAL_VALUE, RANGE);
  softPwmCreate(LW, INITIAL_VALUE, RANGE);
  pinMode(RWT, OUTPUT);
  pinMode(RWL, OUTPUT);
  pinMode(LWT, OUTPUT);
  pinMode(LWL, OUTPUT);
...
void stop_motors()
{
  ...
  digitalWrite(RWL, LOW);
  digitalWrite(LWT, LOW);
  digitalWrite(LWL, LOW);
...
void motors(double speed, double left_offset, double right_offset)
{
  ...
  // left motor
  if (left_speed < 0)  {
    softPwmWrite(LW, (int) -left_speed);
    digitalWrite(LWT, LOW);
    digitalWrite(LWL, HIGH);
  }
  else
  if (left_speed > 0){
    softPwmWrite(LW, (int) left_speed);
    digitalWrite(LWT, HIGH);
    digitalWrite(LWL, LOW);
  }

  // right motor
  if (right_speed < 0)  {
    softPwmWrite(RW, (int) -right_speed);
    digitalWrite(RWT, LOW);
    digitalWrite(RWL, HIGH);
  }
  else
  if (right_speed > 0)  {
    softPwmWrite(RW, (int) right_speed);
    digitalWrite(RWT, HIGH);
    digitalWrite(RWL, LOW);
  }
}
int main()
{
    ...
    //    printf("[AFTER] gyro_scaled_y=%f, deltaT=%lf, rotation_y=%f, last_y=%f\n", (double)gyro_scaled_y, (double)deltaT, (double)rotation_y, (double) last_y);
        ...
        pid();
        printf("%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", error, speed, pTerm, iTerm, dTerm, last_y, last_x);
